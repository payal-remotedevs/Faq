...
class FAQCategoryUtility
{
    ...
    private static function getChildrenCategoriesRecursive(string $idList, int $counter = 0): string
    {
        ...
        $queryBuilder = GeneralUtility::makeInstance(ConnectionPool::class)
            ->getQueryBuilderForTable('tx_faq_domain_model_category');
        $res = $queryBuilder
            ->select('uid')
            ->from('tx_faq_domain_model_category')
            ->where(
        ...
        while (($row = $res->fetchAssociative())) {
            ...
            if ($counter > 10000) {
                GeneralUtility::makeInstance(TimeTracker::class)
                    ->setTSlogMessage('EXT:faq: one or more recursive categories where found');
